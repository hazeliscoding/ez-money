"""Command-line interface.
    ...
    python -m ezmoney import <pdf(s)> [-o out.xlsx]   # build the Excel tracker
    python -m ezmoney parse  <pdf(s)> [--pretty]      # emit cleaned JSON (for the web app)
"""
...
def cmd_parse(args) -> int:
    """Parse PDF(s) and print cleaned transactions as JSON to stdout.

    Diagnostics go to stderr so stdout is pure JSON (the web backend pipes it).
    """
    paths = _expand(args.pdfs)
    if not paths:
        json.dump({"error": "no PDF files matched", "transactions": []}, sys.stdout)
        return 1
    ruleset = load_rules(args.rules)
    txns, periods = [], []
    uncategorized: set[str] = set()
    for p in paths:
        period, raw = parse_pdf(p)
        if period is None:
            print(f"warn: {os.path.basename(p)}: no statement period detected",
                  file=sys.stderr)
            continue
        cleaned, uncat = clean(raw, period, ruleset)
        uncategorized.update(uncat)
        if period not in periods:
            periods.append(period)
        txns.extend(cleaned)
        print(f"parsed {os.path.basename(p)} [{period}]: {len(cleaned)} transactions",
              file=sys.stderr)
    payload = {
        "periods": periods,
        "count": len(txns),
...
def main(argv=None) -> int:
    ...
    imp.add_argument("--rules", default=None, help="path to a custom category_rules.json")
    imp.set_defaults(func=cmd_import)

    par = sub.add_parser("parse", help="parse PDF(s) and emit cleaned JSON to stdout")
    par.add_argument("pdfs", nargs="+", help="PDF path(s) or glob")
    par.add_argument("--rules", default=None, help="path to a custom category_rules.json")
    par.add_argument("--pretty", action="store_true", help="pretty-print the JSON")
